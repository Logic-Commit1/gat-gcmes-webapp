...
class ScopesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create scope" do
    assert_difference("Scope.count") do
      post scopes_url, params: { scope: { name: @scope.name, product_id: @scope.product_id } }
    end
  ...
  test "should update scope" do
    patch scope_url(@scope), params: { scope: { name: @scope.name, product_id: @scope.product_id } }
